...
function Results({ resultOne, resultTwo, matches }: Props) {
  ...
  return (
    <main>
      {matches.length > 0 ?
      <>
      <p className='text-center pb-2 border-b text-[15px] sm:text-base'>It looks like {resultOne.base.name} and {resultTwo.base.name} could have crossed paths in these productions:</p>
      <div className='grid grid-cols-2 lg:grid-cols-3 sm:text-base text-xs'>
      {matches.map((outer) => {
        return (
          <div key={uuidv4()} className='max-w-fit m-auto'>
            <div className='text-sm sm:text-xl font-bold text-center underline'><a href={`${IMDB_URL}${outer[0].id}`}>{outer[0].title} ({outer[0].year})</a></div>
